...
function normalize(p: any) {
  ...
  // Open Food Facts returns energy in kcal as "energy-kcal_100g" (sometimes null)
  ...
  return {
    ...
    brand: p.brands || '',
    serving: '100 g',            // all values are per 100 g
    calories,
    protein_g,
    carbs_g,
    fat_g,
...
export async function GET(req: Request) {
  ...
  const pageSize = Math.min(20, Math.max(1, Number(searchParams.get('limit') || 10)));
  ...
  try {
    const r = await fetch(url, { next: { revalidate: 60 } }); // cache 60s at the edge
    if (!r.ok) throw new Error(`Upstream ${r.status}`);
    const json = await r.json();
    const items = Array.isArray(json?.products) ? json.products.map(normalize) : [];
    // Filter out results that have zero nutrients (common in OFoods)
    const filtered = items.filter(
      (x) => x.calories || x.protein_g || x.carbs_g || x.fat_g
    );
